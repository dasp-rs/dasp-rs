...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (cost, path) = feat::dtw(&x, &y).metric(DtwMetric::Euclidean).compute()?;
    println!("DTW cost:    {cost:.3}");
    println!(
        "DTW path:    {} steps (first 5: {:?})",
        path.len(),
        &path[..path.len().min(5)]
    );
    ...
    println!(
        "Viterbi (loop):    ll={ll:.2}, states[..5]={:?}",
        &states[..5]
    );

    let (ll2, states2) = feat::viterbi(&log_prob, &t_acyclic)?;
    println!(
        "Viterbi (acyclic): ll={ll2:.2}, states[..5]={:?}",
        &states2[..5]
    );
    ...
    println!(
        "Viterbi disc:      ll={ll3:.2}, states[..5]={:?}",
        &states3[..5]
    );
    ...
    let prob1: Vec<f32> = (0..n_frames)
        .map(|t| if t < n_frames / 2 { 0.1 } else { 0.9 })
        .collect();
    let (ll4, states4) = feat::viterbi_binary(&prob1, &log_t2)?;
    println!(
        "Viterbi binary:    ll={ll4:.2}, state[0]={}, state[last]={}",
        states4[0],
        states4[n_frames - 1]
    );
