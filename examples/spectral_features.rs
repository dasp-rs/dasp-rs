...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let chroma_stft = feat::chroma_stft(&y, sr)
        .n_fft(n_fft)
        .hop_length(hop)
        .compute()?;
    ...
    let centroid = feat::spectral(&y, sr)
        .n_fft(n_fft)
        .hop_length(hop)
        .spectral_centroid()?;
    let bandwidth = feat::spectral(&y, sr)
        .n_fft(n_fft)
        .hop_length(hop)
        .spectral_bandwidth()?;
    let flatness = feat::spectral(&y, sr)
        .n_fft(n_fft)
        .hop_length(hop)
        .spectral_flatness()?;
    let rolloff = feat::spectral(&y, sr)
        .n_fft(n_fft)
        .hop_length(hop)
        .spectral_rolloff()?;
    let flux = feat::spectral(&y, sr)
        .n_fft(n_fft)
        .hop_length(hop)
        .spectral_flux()?;
    ...
    // ── Spectral contrast ─────────────────────────────────────────────────────
    let contrast = feat::spectral(&y, sr)
        .n_fft(n_fft)
        .hop_length(hop)
        .spectral_contrast()?;
