...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "YIN mean F0:       {mean_f0:.1} Hz over {} frames",
        f0_yin.len()
    );
    ...
    println!(
        "pYIN mean F0:      {mean_pyin:.1} Hz over {} frames",
        f0_pyin.len()
    );
    ...
    let deviation_cents = pitch::estimate_tuning(&y).sample_rate(sr).compute()?;
